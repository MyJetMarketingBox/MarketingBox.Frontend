...
import {
  getRegFilesSuccess,
  getRegFilesFail,
  uploadFileFail, uploadFileSuccess, getRegDetailFileSuccess, getRegDetailFileFail
} from "./actions";

import { getRegDetailFile, getRegFiles, uploadRegFile } from "../../helpers/backend_helper";
...
function* getRegDetailFileSaga({ nextUrl, filter } : any) {
  try{
    const response : Promise<any> = yield call(getRegDetailFile, nextUrl, filter)
